...
CARBON_PORT = 2003
FPM_URL="http://www.example.com/fpm-status"
...
class Fpm:
    ...
    def fetch_fpm(self):
        ...
        for line in raw_data.split("\n"):
            if line.strip():
                try : metric,i= line.split(":")
                except: continue
                if metric in metrics.keys():
                    data.append("php-fpm.%s %d %d" % (metrics[metric], int(i), now))
        return data
...
def main():
    ...
    while True:
        ...
        for datum in php_fpm.fetch_fpm():
            metric = "app.%s.%s" % (hostname, datum)
            if "-debug" in argv: print metric
            graphite.send(metric)
